...
hist(simulation,probability = TRUE)
lines(density(simulation))
median(simulation)
quantile(simulation)
quantile(simulation,probs=seq(from=0,to=1,by=0.01))
quantile(simulation,probs=0.95)
...
colnames(resultat)=indices_colones
rownames(resultat)=indices_lignes
